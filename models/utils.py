...
def get_idxs_tokens_aggregation(x, side):
    """
    Inputs:
    ...
    side_in = int(math.sqrt(num_patches))
    ...
    tokens_idxs = torch.arange(0,num_patches).reshape(side_in, side_in)
    x_rows = torch.split(tokens_idxs, list(idxs_h.numpy()), dim=0)
    x_patches = []
    for x_row in x_rows:
        x_cols = torch.split(x_row, list(idxs_h.numpy()), dim=1)
